...
const Owner = require("../models/managerModels");
const PasswordReset = require("../models/utils_models/PasswodReset");
...
const updateUserPassword = async (userId, newPassword, req, res) => {
  try {
    ...
    // Check both the Owners and Managers collections for the user with the given ID
    user = await Owner.findById(userId); // Check the Owners collection
    if (!user) {
      user = await Manager.findById(userId); // Check the Managers collection
    }
